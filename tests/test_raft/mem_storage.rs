use raftkv::{HardState, MemberShipConfig, LogIndex, Entry, Storage, StorageResult, InitialState, EntryDetail};
...
impl Storage<(), Action> for MemoryStorage {
    ...
    fn append_entries_to_log(&self, entries: &[Entry<(), Action>]) -> StorageResult<()> {
        let mut inner = self.inner.write();
        inner.logs.extend(entries.iter().map(|entry| (entry.index, entry.clone())));
        Ok(())
    }
