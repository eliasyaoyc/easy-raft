...
impl<N> MemberShipConfig<N> {
    ...
    pub(crate) fn other_voter_members(&self, current: NodeId) -> impl Iterator<Item=(NodeId, Arc<N>)> {
        self.members
            .clone()
            .into_iter()
    ...
    pub(crate) fn add_non_voter(&self, id: NodeId, info: N) -> IResult<Self> {
    ...
    pub(crate) fn remove_node(&self, id: NodeId) -> IResult<Self> {
        if !self.is_member(id) && !self.is_non_voter(id) {
