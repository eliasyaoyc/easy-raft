...
type ReplySender = oneshot::Sender<IResult<()>>;

type AppendEntriesFuture =
BoxFuture<'static, IResult<(u64, u64, u64, AppendEntriesResponse), anyhow::Error>>;
        ...
        D: Send + Unpin + 'static,
{
    pub fn new(
        ...
        storage: Arc<dyn Storage<N, D>>,
        network: Arc<dyn Network<N, D>>,
    ) -> IResult<(Self, mpsc::Sender<Message<N, D>>)> {
    ...
    fn do_send_entries(&mut self, target: NodeId, target_info: Arc<N>) -> IResult<()> {
    ...
    fn send_append_entries(&mut self) -> IResult<()> {
        if self.role != Role::Leader {
            return Ok(());
        }
    ...
    fn switch_to_follower(&mut self, term: u64) -> IResult<()> {
        debug!(
            name = %self.name,
            id = self.id,
            "Become to follower",
    ...
    fn handle_election_timeout(&mut self) -> IResult<()> {
    ...
    fn become_to_leader(&mut self) -> IResult<()> {
        debug!(
            name = %self.name,
            id = self.id,
    ...
    fn handle_vote(&mut self, req: VoteRequest) -> IResult<VoteResponse> {
    ...
    fn append_entry(
        ...
        reply: Option<oneshot::Sender<IResult<()>>>,
    ) -> IResult<()> {
        if self.role != Role::Leader {
    ...
    fn handle_client_read(&mut self, reply: oneshot::Sender<IResult<()>>) -> IResult<()> {
        self.append_entry(EntryDetail::Blank, Some(reply))
    }

    fn handle_client_write(&mut self, action: D, reply: oneshot::Sender<IResult<()>>) -> IResult<()> {
        debug!(
            name = %self.name,
            id = self.id,
            "client_write",
    ...
    fn handle_remove_node(&mut self, id: NodeId, reply: oneshot::Sender<IResult<()>>) -> IResult<()> {
        self.append_entry(
    ...
    fn handle_metrics(&self) -> IResult<Metrics<N>> {
        Ok(Metrics {
            id: self.id,
            role: self.role,
            current_term: self.current_term,
    ...
    fn handle_message(&mut self, msg: Message<N, D>) -> IResult<()> {
        match msg {
    ...
    fn reply_client_write(&mut self, before: u64, res: impl Fn() -> IResult<()>) {
    ...
    fn apply_to_state_machine_if_needed(&mut self) -> IResult<Option<u64>> {
